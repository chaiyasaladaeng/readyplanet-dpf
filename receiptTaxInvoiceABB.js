const receiptTaxInvoiceABBRender = require('./receiptTaxInvoiceABBRender')
const formatCurrency = require('./lib/formatCurrency')

module.exports = function receiptTaxInvoiceABB(PDFDocument, docs, imageLogo) {
   ...
   //  }
    ...
    totalTransactionArray.forEach((transactionArray, index) => {
        ...
        // print page
        documentPDF = receiptTaxInvoiceABBRender(documentPDF, docs, transactionArray, {currentPage: index + 1, totalPage: totalTransactionArray.length}, lastPage)
        // check not equal
        if (index !== totalTransactionArray.length - 1) {
            documentPDF.addPage()
        }
