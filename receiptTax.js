const receiptTaxRender = require('./receiptTaxRender')

module.exports = function receiptTax(PDFDocument,docs, imageLogo,) {
    ...
    totalTransactionArray.forEach((transactionArray, index) => {
        ...
        // print page
        documentPDF = receiptTaxRender(documentPDF, docs, transactionArray, {currentPage: index + 1, totalPage: totalTransactionArray.length}, lastPage)
        // check not equal
        if (index !== totalTransactionArray.length - 1) {
            documentPDF.addPage()
        }
